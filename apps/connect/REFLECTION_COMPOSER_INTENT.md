...
AI is a collaborator, not an autopilot. The Sparkle button is always available. It opens the Hints sheet so the Companion can tell the AI what it needs to know:

- "I'm in this photo."
- "The Explorer is in this photo."
- "That's Nona, dog Dalton, baby Dante, and we're at Nona's house."

Then the Companion taps Run Sparkle and the AI generates a caption and deep dive informed by that context. If the result is wrong, the Companion adjusts hints and Sparkles again. Or edits the text directly. Or ignores AI entirely and writes from scratch.

The only time AI runs automatically is on new content with no caption — and even then, the Hints sheet opens first so the Companion can set context before AI fires. For edits (where a caption already exists), AI never runs unless the Companion explicitly asks.

If the Companion changes something meaningful (trim, thumbnail, caption, filter) after the last Sparkle run and tries to send or preview, the editor gently asks: "You've made changes — want to re-run Sparkle?" This is not a gate. It is a reminder. The Companion can always send anyway.

## The 80/20 Fast Path

Most new Reflections will be quick: pick a photo, glance at the hints (pre-filled from the confirmation screen), tap Run Sparkle, review the AI caption, send. Three taps. The editor does not fight this. The hints sheet is lightweight. The defaults are good. The fast path is fast.

But when a Companion wants to craft something special — a birthday video, a family reunion photo, a moment with a new pet — every tool is there. No hidden menus. No "advanced mode." The same editor, the same buttons, the same flow. Just used with more intention.
